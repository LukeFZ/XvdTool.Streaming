// source: https://github.com/Thealexbarney/LibHac/blob/master/src/LibHac/Crypto/Impl/AesXtsCipherNi.cs
...
public struct AesXtsCipherNi
{
    private AesCoreNi _dataAesCore;
    private AesCoreNi _tweakAesCore;

    public void Initialize(ReadOnlySpan<byte> key1, ReadOnlySpan<byte> key2, bool decrypting)
    {
        _dataAesCore.Initialize(key1, decrypting);
        _tweakAesCore.Initialize(key2, false);
    }

    public int Encrypt(ReadOnlySpan<byte> input, Span<byte> output, ReadOnlySpan<byte> tweakIv)
    {
        Debug.Assert(tweakIv.Length == 16);
    ...
    public int Decrypt(ReadOnlySpan<byte> input, Span<byte> output, ReadOnlySpan<byte> tweakIv)
    {
        ...
        if (leftover != 0)
        {
            Debug.Assert(false, "We should never attempt to decrypt a non 0x1000 aligned block");
